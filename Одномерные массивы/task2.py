...
for i in range(arr_size):
    # Если текущий элемент больше текущего максимума, то максимум обновляем. На первом шаге сравниваем текущий элемент с нулём
    if array[i] > max:
        max = array[i]

    # Если текущий элемент меньше текущего минимума, то минимум обновляем. На первом шаге сравниваем текущий элемент со значением arr_max
    if array[i] < min:
        min = array[i]
